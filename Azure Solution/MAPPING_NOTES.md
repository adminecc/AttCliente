...
| 2026-07-03 | Consulta de solicitud | El prototipo de consulta seguia usando mocks locales y la API solo aceptaba `email + token`. | Solucionado | `POST /api/solicitudes/consultar` acepta `token + personalData` (correo o telefono), devuelve contrato de pantalla (`caseId`, `status`, fechas, adjuntos) y mantiene campos heredados. |
